...
Specifies the DNS server to use for queries. If not specified, the system's default DNS server will be used.

.PARAMETER HostsFormat

Formats the output as a hosts file entry (IP hostname) instead of a PowerShell object. Useful for directly appending to /etc/hosts or C:\Windows\System32\drivers\etc\hosts.
...
Invoke-ReverseDnsLookup '74.125.228.1,74.125.228.4-74.125.228.6' -DNSServer 8.8.8.8
...
Description
-----------
Returns the hostnames of the IP addresses using Google's DNS server (8.8.8.8).
...
Description
-----------
Returns the results in hosts file format for easy copying to /etc/hosts.

.EXAMPLE

Invoke-ReverseDnsLookup 10.10.10.0/24 -DNSServer 10.10.10.1 -HostsFormat | Out-File -Append C:\Windows\System32\drivers\etc\hosts

Description
-----------
Queries an internal DNS server and appends the results directly to the Windows hosts file.

.EXAMPLE

Write-Output "74.125.228.1,74.125.228.0/29" | Invoke-ReverseDnsLookup -DNSServer 1.1.1.1
...
Description
-----------
Returns the hostnames using Cloudflare's DNS server (1.1.1.1).
...
#>
    ...
    BEGIN {
        ...
        if ($DNSServer) {
            $DnsIp = $null
            if (-not [System.Net.IPAddress]::TryParse($DNSServer, [ref]$DnsIp)) {
                Write-Warning "Invalid DNS server IP address supplied!"
                return
            }
            Write-Verbose "Using DNS server: $DNSServer"
        }
    }
    ...
    PROCESS {
        Parse-IPList $IpRange | ForEach-Object {
            try {
                ...
                if ($DNSServer) {
                    # Use nslookup with specified DNS server
                    $NslookupResult = nslookup $_ $DNSServer 2>$null
                    $HostName = $null
                    ...
                    if ($HostsFormat) {
                        Write-Output "$_`t$($Temp.HostName)"
                    } else {
                        $Result = @{
                            IP = $_
